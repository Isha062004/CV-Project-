...
# normalize model and prototxt paths
args["prototxt"] = os.path.abspath(args["prototxt"])
...
try:
	...
	input_src = os.path.abspath(input_src)

if isinstance(input_src, int):
	vs = VideoStream(src=input_src).start()
	# warm up the camera for a couple of seconds
	time.sleep(2.0)
else:
	if not os.path.isfile(input_src):
		raise FileNotFoundError(f"Input video file not found: {input_src}")
	vs = cv2.VideoCapture(input_src)
...
while True:
	# grab the frame from the video source and resize it to have a maximum width of 400 pixels
